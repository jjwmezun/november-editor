...
import { getOverworldTypeFactory, getOverworldTypeGenerator } from '../../../../common/objects';
...
const createObjectRenderer = (
	...
	program.setUniform1f( `u_alpha`, 1.0 );
...
const generateRenderer = (
	...
	const objectRenderers = [ createObjectRenderer( ctx, palettes, graphics, selectedPalette ) ];

	objectRenderers.forEach( ( objectRenderer: OverworldObjectRenderer ) => {
		objectRenderer.updateObjects( overworld.getObjectsList() );
...
function OverworldGridCanvas( props: OverworldGridCanvasProps ): ReactElement {
	...
	const objects = overworld.getObjectsList();
	...
	const onRightClick = ( e: SyntheticBaseEvent ) => {
		...
		setOverworld( overworld.addObject( typesFactory( selectedObjectType, gridX, gridY ) ) );
	};

	useEffect( () => {
		if ( canvasRef.current ) {
			const newRenderer = generateRenderer( canvasRef.current, overworld, graphics, palettes, 2 );
	...
	useEffect( () => {
		...
		renderer.updateLayerObjects( 0, overworld.getObjectsList(), selectedObject );
		renderer.render();
	}, [ overworld.getObjectsList(), renderer ] );
